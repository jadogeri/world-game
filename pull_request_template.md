...
- [ ] If `packages/api-spec/openapi.yaml` changed, ran `pnpm --filter @repo/api-spec run codegen` and committed the generated output
- [ ] If `packages/db` schema changed, ran `pnpm --filter @repo/db run push` (dev) and documented the change
